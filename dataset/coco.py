import os
import random
import numpy as np
import torch
from PIL import Image

from torch.utils.data import Dataset
import cv2

try:
    from pycocotools.coco import COCO
except:
    print("It seems that the COCOAPI is not installed.")

try:
    from .transforms import mosaic_augment, StrongTransforms
except:
    from transforms import mosaic_augment


coco_class_labels = ('person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle')
# coco_class_labels = ('car',)

coco_class_color = [(99,178,238),(118,218,145),(248,203,127),(248,149,136),(124,214,207),(145,146,171),(148,60,57),(98,76,124)]
# coco_class_color = [(248,203,127),]

# coco_class_index = [1, 2, 3, 4, 5, 6, 7, 8]
coco_class_index = [0,1, 2, 3, 4, 5, 6, 7]
# coco_class_index = [0, ]

class COCODataset(Dataset):
    """
    COCO dataset class.
    """
    def __init__(self, 
                 img_size=640,
                 data_dir=None, 
                 image_set='train2017',
                 transform=None,
                 color_augment=None,
                 mosaic=False,
                 strong=None,
                 weak=None,
                 is_train=True):
        """
        COCO dataset initialization. Annotation data are read into memory by COCO API.
        Args:
            data_dir (str): dataset root directory
            json_file (str): COCO json file name
            name (str): COCO data name (e.g. 'train2017' or 'val2017')
            debug (bool): if True, only one data id is selected from the dataset
        """
        if image_set == 'train':
            self.set_type = 'train'
            self.json_file='instances_train.json'
        elif image_set == 'val':
            self.set_type = 'val'
            # self.json_file = 'val_modified_caronly.json'
            self.json_file='instances_foggy_val.json'
            # self.json_file = 'instance_ele_val.json'
        elif image_set == 'car_val':
            self.set_type = 'val'
            self.json_file='instances_val.json'
        elif image_set == 'tar_train':
            self.set_type = 'train'
            self.json_file = 'instances_foggy_train.json'
        elif image_set == 'kitti_train':
            self.set_type = 'train'
            self.json_file = 'train_kitti_caronly.json'
        elif image_set == 'city_train':
            self.set_type = 'train'
            self.json_file = 'train_modified_caronly.json'
        elif image_set == 'sim10k_train':
            self.set_type = 'train'
            self.json_file = 'train_sim10k_caronly.json'
        elif image_set == 'coco_train':
            self.set_type = 'train'
            self.json_file = 'instances_coco_train.json'
        elif image_set == 'elevator_train':
            self.set_type = 'train'
            self.json_file = 'instance_ele_train.json'


        self.img_size = img_size
        self.image_set = image_set
        self.data_dir = data_dir
        self.coco = COCO(os.path.join(self.data_dir, 'annotations', self.json_file))
        self.ids = self.coco.getImgIds()
        self.class_ids = sorted(self.coco.getCatIds())
        self.img_ids = self.get_imgIds()
        self.img_infos = self.get_imgInfos()
        self.is_train = is_train
        # augmentation
        self.transform = transform
        self.strong = strong
        self.weak = weak
        self.color_augment = color_augment
        self.mosaic = mosaic
        if self.mosaic:
            print('use Mosaic Augmentation ...')



    def __len__(self):

        return len(self.ids)


    def __getitem__(self, index):
        image,weak_image, target, mask = self.pull_item(index)
        return image,weak_image, target,mask


    def get_imgInfos(self):
        """
        获取图片的信息
        :return:
        """
        img_infos = []
        for i in self.img_ids:
            info = self.coco.loadImgs(i)[0]
            img_infos.append(info)
        return img_infos

    def get_image_path(self, index):
        """
                获取图片的path
                :return:
                """
        img_info = self.img_infos[index]
        id_ = self.ids[index]
        img_name = img_info["file_name"]
        img_file = os.path.join(self.data_dir, 'images', self.set_type,
                                img_name)


        return img_file


    def load_image_target(self, index):
        img_info = self.img_infos[index]
        anno_ids = self.coco.getAnnIds(imgIds=img_info["id"], iscrowd=0)
        annotations = self.coco.loadAnns(anno_ids)

        img_name = img_info["file_name"]
        # load an image
        img_file = os.path.join(self.data_dir,'images', self.set_type,
                                img_name)
        image = cv2.imread(img_file)
        
        if self.json_file == 'instances_val5k.json' and image is None:
            img_file = os.path.join(self.data_dir, 'train2017',
                                    '{:012}'.format(index) + '.jpg')
            image = cv2.imread(img_file)

        assert image is not None

        height, width, channels = image.shape
        
        #load a target
        anno = []
        for label in annotations:
            if 'bbox' in label and label['area'] > 0:   
                xmin = np.max((0, label['bbox'][0]))
                ymin = np.max((0, label['bbox'][1]))
                xmax = np.min((width - 1, xmin + np.max((0, label['bbox'][2] - 1))))
                ymax = np.min((height - 1, ymin + np.max((0, label['bbox'][3] - 1))))
                if xmax > xmin and ymax > ymin:
                    label_ind = label['category_id']
                    cls_id = self.class_ids.index(label_ind)
                    anno.append([xmin, ymin, xmax, ymax, cls_id])  # [xmin, ymin, xmax, ymax, label_ind]
            else:
                print('No bbox !!!')

        # guard against no boxes via resizing
        anno = np.array(anno).reshape(-1, 5)
        target = {
            "boxes": anno[:, :4],
            "labels": anno[:, 4],
            "orig_size": [height, width]
        }

        return image, target


    def pull_item(self, index):
        # load a mosaic image
        if self.mosaic and np.random.randint(2):
            ids_list_ = self.ids[:index] + self.ids[index+1:]
            # random sample other indexs
            id1 = self.ids[index]
            id2, id3, id4 = random.sample(ids_list_, 3)
            ids = [id1, id2, id3, id4]

            image_list = []
            target_list = []
            # load image and target
            for id_ in ids:
                img_i, target_i = self.load_image_target(id_)
                image_list.append(img_i)
                target_list.append(target_i)

            image, target = mosaic_augment(image_list, target_list, self.img_size)

            # augment
            # image, target, mask = self.color_augment(image, target)
        # load an image and target
        else:
            img_id = self.ids[index]
            image, target = self.load_image_target(index)
            w_image= image.copy()
            w_target = target.copy()
            if self.weak is None:
                image_pil = Image.fromarray(image.astype("uint8"), "RGB")
                image = np.array(self.strong(image_pil))
                weak_image = torch.tensor(0)
                image, target, mask = self.transform(image, target)
            else:
                image_pil = Image.fromarray(image.astype("uint8"), "RGB")
                image = np.array(self.strong(image_pil))
                image, target, mask = self.transform(image, target)
                weak_image,_,_ = self.transform(w_image, w_target)
            # augment

            return image,weak_image, target, mask







    def pull_image(self, index):
        img_info = self.img_infos[index]
        id_ = self.ids[index]
        img_name = img_info["file_name"]
        img_file = os.path.join(self.data_dir, 'images', self.set_type,
                                img_name)
        img = cv2.imread(img_file)

        if self.json_file == 'instances_val5k.json' and img is None:
            img_file = os.path.join(self.data_dir, 'train2017',
                                    '{:012}'.format(id_) + '.jpg')
            img = cv2.imread(img_file)

        return img, id_


    def pull_anno(self, index):
        # id_ = self.ids[index]
        img_info = self.img_infos[index]
        anno_ids = self.coco.getAnnIds(imgIds=img_info["id"], iscrowd=None)
        annotations = self.coco.loadAnns(anno_ids)
        
        anno = []
        for label in annotations:
            if 'bbox' in label:
                xmin = np.max((0, label['bbox'][0]))
                ymin = np.max((0, label['bbox'][1]))
                xmax = xmin + label['bbox'][2]
                ymax = ymin + label['bbox'][3]
                
                if label['area'] > 0 and xmax >= xmin and ymax >= ymin:
                    label_ind = label['category_id']
                    cls_id = self.class_ids.index(label_ind)

                    anno.append([xmin, ymin, xmax, ymax, cls_id])  # [xmin, ymin, xmax, ymax, label_ind]
            else:
                print('No bbox !!')
        return anno

    def get_imgIds(self):
        """
        得到数据集中的图片id，对于test模式下，不需要过滤图片；对于train模式，需要过滤图片，保留含有物体的图片
        :return:
        """
        #
        # if(not self.is_train):
        #     return self.ids
        # vailed_img_ids=[]
        # for i in range(len(self.ids)):
        #     annids = self.coco.getAnnIds(self.ids[i],iscrowd=False)
        #     if(len(annids)==0):
        #         continue
        #     vailed_img_ids.append(self.ids[i])
        return self.ids






if __name__ == "__main__":
    from dataset.transforms import TrainTransforms, BaseTransforms, build_strong_augmentation, StrongTransforms
    # format = 'BGR'
    # pixel_mean = [103.53, 116.28, 123.675]
    # pixel_std = [1.0, 1.0, 1.0]

    format = 'RGB'
    pixel_mean = [0.485, 0.456, 0.406]
    pixel_std = [0.229, 0.224, 0.225]

    # trans_config = [{'name': 'DistortTransform',
    #                  'hue': 0.1,
    #                  'saturation': 1.5,
    #                  'exposure': 1.5},
    #                 {'name': 'RandomHorizontalFlip'},
    #                 {'name': 'RandomShift', 'max_shift': 32},
    #                 {'name': 'JitterCrop', 'jitter_ratio': 0.3},
    #                 {'name': 'ToTensor'},
    #                 {'name': 'Resize'},
    #                 {'name': 'Normalize'},
    #                 {'name': 'PadImage'}]
    trans_config = [{'name': 'RandomHorizontalFlip'},
                    {'name': 'RandomShift', 'max_shift': 32},
                    {'name': 'ToTensor'},
                    {'name': 'Resize'},
                    {'name': 'Normalize'},
                    {'name': 'PadImage'}]
    min_size = 600
    max_size = 1333
    random_size = None
    train_transform = StrongTransforms(trans_config=trans_config,
                                       min_size=min_size,
                                       max_size=max_size,
                                       random_size=random_size,
                                       pixel_mean=pixel_mean,
                                       pixel_std=pixel_std,
                                       format=format)
    weak_augment = TrainTransforms(trans_config=trans_config,
                                       min_size=min_size,
                                       max_size=max_size,
                                       random_size=random_size,
                                       pixel_mean=pixel_mean,
                                       pixel_std=pixel_std,
                                       format=format)
    color_augment = BaseTransforms(min_size=max_size,
                                   max_size=max_size,
                                   random_size=random_size,
                                   pixel_mean=pixel_mean,
                                   pixel_std=pixel_std,
                                   format=format)
    pixel_mean = np.array(pixel_mean, dtype=np.float32)
    pixel_std = np.array(pixel_std, dtype=np.float32)


    dataset = COCODataset(img_size=max_size,
                          data_dir='D:\\datasets\\cityscapes',
                          image_set='train',
                          transform=train_transform,
                          color_augment=color_augment,
                          mosaic=False,
                          strong=build_strong_augmentation())
    tar_dataset = COCODataset(img_size=max_size,
                              data_dir='D:\\datasets\\foggy_cityscapes',
                              image_set='tar_train',
                              transform=train_transform,
                              color_augment=color_augment,
                              mosaic=False,
                              weak=weak_augment,
                              strong=build_strong_augmentation())
    
    np.random.seed(0)
    class_colors = [(np.random.randint(255),
                     np.random.randint(255),
                     np.random.randint(255)) for _ in range(80)]
    print('Data length: ', len(dataset))

    for i in range(1000):
        image,weak_image, target, mask = dataset.pull_item(i)
        print(image.size())
        # to numpy
        image = image.permute(1, 2, 0).numpy()
        # weak_image = weak_image.permute(1, 2, 0).numpy()
        # to BGR format
        if format == 'RGB':
            # denormalize
            image = image * pixel_std + pixel_mean
            image = image * 255
            image = image[:, :, (2, 1, 0)].astype(np.uint8)
            # weak_image = weak_image * pixel_std + pixel_mean
            # weak_image = weak_image * 255
            # weak_image = weak_image[:, :, (2, 1, 0)].astype(np.uint8)
        elif format == 'BGR':
            image = image * pixel_std + pixel_mean
            image = image.astype(np.uint8)
        image = image.copy()
        # weak_image = weak_image.copy()
        img_h, img_w = image.shape[:2]

        boxes = target["boxes"]
        labels = target["labels"]

        for box, label in zip(boxes, labels):
            x1, y1, x2, y2 = box
            cls_id = int(label)
            color = class_colors[cls_id]
            # class name
            label = coco_class_labels[coco_class_index[cls_id]]
            image = cv2.rectangle(image, (int(x1), int(y1)), (int(x2), int(y2)), (0,0,255), 2)
            # weak_image = cv2.rectangle(weak_image, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)
            # put the test on the bbox
            cv2.putText(image, label, (int(x1), int(y1 - 5)), 0, 0.5, color, 1, lineType=cv2.LINE_AA)
            # cv2.putText(weak_image, label, (int(x1), int(y1 - 5)), 0, 0.5, color, 1, lineType=cv2.LINE_AA)
        cv2.imshow('gt', image)
        # cv2.imwrite(str(i)+'.jpg', img)
        cv2.waitKey(0)
        # cv2.imshow('weak', weak_image)
        # cv2.waitKey(0)

        if mask is not None:
            # to numpy
            mask = mask.cpu().numpy()
            mask = (mask * 255).astype(np.uint8).copy()

            boxes = target["boxes"]
            labels = target["labels"]
            for box, label in zip(boxes, labels):
                x1, y1, x2, y2 = box
                cv2.rectangle(mask, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), 2)

            cv2.imshow('mask', mask)
            cv2.waitKey(0)
