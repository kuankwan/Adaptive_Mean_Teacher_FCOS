...
class Resize(object):
    ...
    def __call__(self, image, target=None, mask=None):
        if self.min_size is None:
            ...
            if img_w0 < img_h0:
                ow = min_size
                oh = int(min_size * img_h0 / img_w0)
            else:
                oh = min_size
                ow = int(min_size * img_w0 / img_h0)

            # resized_image = F.resize(image, size=min_size,max_size=self.max_size)
            resized_image = F.resize(image, size=min_size)
            # resized_image = F.resize(image, size=[self.min_size, self.max_size])
...
class PadImage(object):
    ...
    def __call__(self, image, target=None, mask=None):
        img_h0, img_w0 = image.shape[1:]
        new_h = int(math.ceil(img_h0 / self.size_divisor)) * self.size_divisor
        new_w = int(math.ceil(img_w0 / self.size_divisor)) * self.size_divisor
        # pad_image = torch.zeros([image.size(0), self.max_size, self.max_size]).float()
        pad_image = torch.zeros([image.size(0), new_h, new_w]).float()
...
def build_strong_augmentation():
    ...
    augmentation.append(transforms.RandomGrayscale(p=0.2))
    # augmentation.append(transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.5, 2.0), shear=45))
    augmentation.append(transforms.RandomApply([GaussianBlur([0.1, 2.0])], p=0.5))
    randcrop_transform = transforms.Compose(
                    [
                        transforms.ToTensor(),
    ...
    augmentation.append(randcrop_transform)
    # augmentation.append(JitterCrop(jitter_ratio=0.3))
    # augmentation.append(transforms.Resize(max_size=self.max_size))
    # augmentation.append(Resize(min_size=self.min_size,
    #                            max_size=self.max_size,
    #                           ))
    # augmentation.append(Normalize(pixel_mean=self.pixel_mean,
    #                             pixel_std=self.pixel_std))
    # augmentation.append(PadImage(max_size=self.max_size))
...
def build_weak_augmentation():
    ...
    # augmentation.append(transforms.RandomApply([transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)], p=0.8))
    # augmentation.append(transforms.RandomGrayscale(p=0.2))
    # augmentation.append(transforms.RandomSolarize(threshold=128))
    # augmentation.append(transforms.RandomAffine(degrees=0, translate=(0.1, 0.1), scale=(0.5, 2.0), shear=45))
    # augmentation.append(transforms.RandomApply([GaussianBlur([0.1, 2.0])], p=0.5))
    randcrop_transform = transforms.Compose(
                    [
