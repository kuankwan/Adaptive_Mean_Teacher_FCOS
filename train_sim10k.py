    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        for step, num_iter in enumerate(bar):
            ...
            if epoch > cfg['open_ins']:
                loss_dict = model(images, mask=masks, targets=targets, tar_x=t_images, open_ins=True, visual=visual)
            else:
                loss_dict = model(images, mask=masks, targets=targets, tar_x=t_images, visual=visual)

            # zeta = 2 / (1+math.exp(-cfg['epsilon'] * ni/total_epoch)) - 1
            # local_loss = zeta * loss_dict['local_loss']
            # global_loss = zeta * loss_dict['global_loss']
            # loss_dict.update({'global_loss': global_loss})
            # loss_dict.update({'local_loss': local_loss})
        ...
        if (epoch) % args.eval_epoch == 0 or (epoch) == max_epoch:
            ...
            if distributed_utils.is_main_process():
                if evaluator is None:
                    ...
                    if map50 > best_map or map50 < best_map:
                        # update best-map
                        best_map = map50
                        # save model
