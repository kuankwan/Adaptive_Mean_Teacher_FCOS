...
class VOCAPIEvaluator():
    ...
    def evaluate(self, net,logger=None):
        net.eval()
        ...
        for i in range(num_images):
            ...
            bboxes, scores, cls_inds,_ = net(x)
            ...
            if (i + 1) == num_images:
                ...
                if logger is not None:
                    logger.info('[Eval: %d / %d]' % (i, num_images))
                    logger.info(
                        f'Overall fps: {fps:.1f} img / s, '
                        f'times per image: {1000 / fps:.1f} ms / img',
                    )
        ...
        self.evaluate_detections(self.all_boxes,logger)
    ...
    def do_python_eval(self, use_07=True,logger=None):
    ...
    def evaluate_detections(self, box_list,logger=None):
        self.write_voc_results_file(box_list)
        self.do_python_eval(logger=logger)
