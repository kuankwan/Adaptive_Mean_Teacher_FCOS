...
from .extra_module import (Conv2dStaticSamePadding, MaxPool2dStaticSamePadding,
...
class SeparableConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels=None, norm='bn', activation='swish', onnx_export=False):
...
class BiFPN(nn.Module):
    def __init__(self, num_channels, conv_channels,norm='bn', first_time=False, epsilon=1e-4, onnx_export=False, attention=True):
        print("num_channels:", num_channels)
        print("conv_channels:", conv_channels)
        print("first_time:", first_time)
        super(BiFPN, self).__init__()
        self.epsilon = epsilon
        self.conv6_up = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)
        self.conv5_up = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)
        self.conv4_up = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)
        self.conv3_up = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)

        self.conv4_down = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)
        self.conv5_down = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)
        self.conv6_down = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)
        self.conv7_down = SeparableConvBlock(num_channels, norm=norm, onnx_export=onnx_export)
