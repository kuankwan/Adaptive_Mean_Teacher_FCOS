...
try:
    from models.backbone.vision import da_resnet
except:
    from models.backbone.vision import model_resnet
...
class Backbone(nn.Module):
    ...
    def __init__(self,
        ...
        if norm_type == 'BN':
            ...
            norm_layer = FrozenBatchNorm2d
        self.backbone = getattr(da_resnet, name)(
            replace_stride_with_dilation=[False, False, dilation],
            pretrained=pretrained, norm_layer=norm_layer)
        num_channels = 512 if name in ('resnet18', 'resnet34') else 2048
...
if __name__ == '__main__':
    model, feat_dim = build_resnet(model_name='resnet50', pretrained=True)
    print(feat_dim)
