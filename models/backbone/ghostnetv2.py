...
class GhostNet(nn.Module):
    ...
    def __init__(self, model_cfgs, multiplier=1.):
        ...
        self.classifier = nn.Linear(
            model_cfgs['cls_ch_expand'], 1000, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.act1(x)
        x = self.blocks[0:4](x)
        layer2 = self.blocks[4](x)
        x = self.blocks[4:10](x)
        layer3 = self.blocks[10](x)
        x = self.blocks[11:15](x)
        layer4 = self.blocks[15](x)
...
def Ghostv2(model_name="1x",pretrained=True):
    ghostnetv2_1x = ghostnetv2(model_name)
    if pretrained:
        print('Loading pretrained Ghostv2 ...')
        # checkpoint = mindspore.load('D:\\python_project\\Categroy-relate attention alignment\\weights\\ck_ghostnetv2_10.pth.tar')
        # ghostnetv2_1x.load_state_dict(checkpoint)
        param_dict = torch.load('D:\\python_project\\Categroy-relate attention alignment\\weights\\ghostv2.pth',map_location='cpu')
        # with open('./pthmodel.txt',mode='w') as file:
        #     for k,v in ghostnetv2_1x.state_dict().keys():
        #         file.writelines(str(k)+'\n')

        ghostnetv2_1x.load_state_dict(param_dict,strict=False)

    return ghostnetv2_1x,[40*4,112*4,160*4]
...
if __name__=='__main__':
    ...
    def forward(self, x):
        ...
        x = self.blocks[4:10](x)
        layer3 = self.blocks[10](x)
        x = self.blocks[10:15](x)
        layer4 = self.blocks[15](x)
        # x = self.global_pool(x)
        # layer4 = self.conv_head(x)
        return {'layer2':layer2,'layer3':layer3,'layer4':layer4}