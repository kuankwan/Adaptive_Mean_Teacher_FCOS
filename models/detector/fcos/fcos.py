...
from ...da.DA import D_layer4, D_layer3, D_layer2, _InstanceDA,Global_D
...
from models.detector.fcos.selfattention import self_attn2
...
class FCOS(nn.Module):
    def __init__(self,
        ...
        self.satten = nn.Parameter(torch.ones(self.num_classes,dtype=torch.float32),requires_grad=True)
        self.tatten = nn.Parameter(torch.ones(self.num_classes, dtype=torch.float32), requires_grad=True)
    ...
    def forward(self, x, mask=None, targets=None, tar_x=None, open_ins=False, visual=False):
        if not self.trainable:
            ...
            for level, feat in enumerate(pyramid_feats):
                ...
                reg_pred = self.reg_pred(reg_feat[-1])
                ctn_pred = self.ctn_pred(reg_feat[-1])
                ctn = ctn_pred.sigmoid()

                if level < 7:
                    s_weight = torch.sigmoid(cls_pred.sigmoid() * ctn_pred.sigmoid() * self.theta)
                    # s_weight = (s_weight - torch.min(s_weight)) / (torch.max(s_weight) - torch.min(s_weight))
...
#                         all_tar_cls_preds.append(tar_cls_preds)
                        ...
                        t_global_list = []
                        for i in range(self.num_classes):
                            s_ins_pred = self.s_discriminators[i](feat * s_weight[0, i:i + 1, :, :] + feat)
                            source_list.append(s_ins_pred)
                            t_ins_pred = self.s_discriminators[i](tar_feat * t_weight[0, i:i + 1, :, :] + tar_feat)
            ...
            if tar_x is not None:
                ...
                pga_loss = pga_loss / 3 * self.alpha
                ...
                if tar_feats_iter is not None and open_ins:
                    ...
                    global_loss = F.binary_cross_entropy_with_logits(s_global_all,torch.full(s_global_all.shape,1.0,dtype=torch.float,device=self.device),reduction="mean") + F.binary_cross_entropy_with_logits(t_global_all,torch.full(t_global_all.shape,0.0,dtype=torch.float,device=self.device),reduction='mean')
